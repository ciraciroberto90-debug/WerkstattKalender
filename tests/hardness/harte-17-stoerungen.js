...
(async () => {
  ...
  // Beide pollen -> jeder muss beide Störungen sehen
  ...
  // Lösch-Knopf der Presse-3-Karte (× im Kartenfuß)
  const presseKarte = u1.locator('div', { hasText: 'Presse 3' }).filter({ has: u1.getByRole('button', { name: 'Störung löschen' }) }).last();
  await presseKarte.getByRole('button', { name: 'Störung löschen' }).click();
